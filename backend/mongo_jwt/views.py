...
from mongo_jwt.utils import create_unique_object_id, pwd_context
from mongo_jwt.db import database, auth_collection, fields, jwt_life, jwt_secret, secondary_username_field, id_field
import jwt
import datetime
from mongo_jwt import messages
...
def signup(request):
    try:
        ...
        if database[auth_collection].find_one({id_field: signup_data[id_field]}) is None:
            ...
            return Response(status=status.HTTP_405_METHOD_NOT_ALLOWED,
                            data={"data": {"error_msg": messages.user_exists_field(id_field)}})
...
def login(request):
    try:
        data = request.data if request.data is not None else {}
        username = data[id_field]
        password = data['password']
        if id_field in data:
            user = database[auth_collection].find_one({id_field: username}, {"_id": 0})
        else:
            if secondary_username_field:
